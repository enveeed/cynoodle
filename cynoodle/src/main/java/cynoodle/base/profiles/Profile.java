...
import cynoodle.base.fm.FMPreferences;
...
import static cynoodle.mongodb.fluent.FluentValues.fromLocalDate;
import static cynoodle.mongodb.fluent.FluentValues.toLocalDate;
...
public final class Profile extends UEntity {
    ...
    public MessageEmbed createEmbed(@Nonnull DiscordPointer guildP) {
        ...
        ShardManager discord = Module.get(DiscordModule.class).getAPI();

        List<ProfileBadge> badgesAll = new ArrayList<>(Module.get(ProfilesModule.class).getBadges());
        badgesAll.sort(Comparator.comparingLong(x -> x.getEmote().getID()));
        List<String> badges = new ArrayList<>();
        for (ProfileBadge badge : badgesAll) {
            if(badge.test(user)) {
                Emote emote = discord.getEmoteById(badge.getEmote().getID());
                if(emote == null) continue;
                badges.add(emote.getAsMention());
            }
        }

        if(badges.size() > 0) {
            personalOut.add(Joiner.on("").join(badges));
        }
