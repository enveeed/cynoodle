...
public final class LaunchSettings {
    ...
    private static final Option OPT_LOG_LEVEL
            = Option.newValueOption("log",'l');

    private static final Option OPT_NO_PERMISSIONS
            = Option.newFlagOption("no-permissions",null);

    private static final Option OPT_SETUP_TEST
            = Option.newFlagOption("setup-test",null);

    private static final Option OPT_CONFIG
            = Option.newFlagOption("config",'c');
    ...
    public static LaunchSettings of(@Nonnull String[] input) {
        ...
        OptionsResult result = options.parse(parameters);
