...
import cynoodle.test.access.AccessModule;
import cynoodle.test.commands.CommandsModule;
import cynoodle.test.fm.FMModule;
import cynoodle.test.local.LocalModule;
import cynoodle.test.makeme.MakeMeModule;
import cynoodle.test.moderation.ModerationModule;
import cynoodle.test.notifications.NotificationsModule;
import cynoodle.test.permissions.PermissionsModule;
import cynoodle.test.profiles.ProfilesModule;
import cynoodle.test.spamfilter.SpamFilterModule;
import cynoodle.test.utilities.UtilitiesModule;
...
public final class CyNoodle {
    ...
    public static void main(@Nonnull String[] args) {
        ...
        // initialize logging (with carambola)
        Carambola.init();


        // setup fallback exception handlers
        Thread.UncaughtExceptionHandler handler = (thread, e) -> LOG.atSevere().withCause(e)
                .log("Uncaught exception in thread %s!", thread.getName());

        Thread.setDefaultUncaughtExceptionHandler(handler);
        Thread.currentThread().setUncaughtExceptionHandler(handler);

        // launch cynoodle
        launch(LaunchSettings.of(args));
    }
