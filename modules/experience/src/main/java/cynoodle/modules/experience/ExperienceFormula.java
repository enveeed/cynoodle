...
import cynoodle.util.MoreMath;

/**
 * Calculates the experience and level relationship.
 */
public final class ExperienceFormula {
    ...
    public static int getReachedLevel(long xp) {
        int level = 0;
        while (getRequiredValue(level) <= xp) level ++;
        if(level == 0) return 0;
        else return level - 1;
    }

    public static long getRequiredValue(int level) {
        return f(level);
    }
